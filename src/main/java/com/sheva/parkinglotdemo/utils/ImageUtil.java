...
public class ImageUtil {
     ...
     * 内核大小 必须是 正奇数
     */
    public static final int GS_BLUR_KERNEL = 3;
    /**
     * 均值滤波算法相关参数
     * 内核大小 必须是 正奇数
     */
    ...
    public static final int DEFAULT_MORPH_SIZE_HEIGHT = 10;

    /**
     * 保存算法每个步骤的处理结果 输出jpg图像
     * @param debug 是否调试
     * @param tempPath 缓存的目录
     * @param methodName 方法名
     * @param inMat 矩阵
     */
    public static void debugImg(Boolean debug, String tempPath, String methodName, Mat inMat){
        if (debug) {
            //通过id生成文件名称，使得每个步骤生成的图片能够按照执行时间进行排序
     ...
     * 边缘检测算子 是一阶梯度算法
     * 所谓梯度运算就是对图像中的像素点进行导数运算，从而得到相邻两个像素点的差异值
     * 对噪声具有平滑作用，提供较为青雀的边缘方向信息，边缘定位精度不够高。当对精度要求不是很高的时候，是一种较为常用的边缘检测方式
     * @param inMat 灰度图
     * @param dst
     * @param debug 是否调试
     * @param tempPath 路径
    ...
    public static void sobel(Mat inMat, Mat dst, Boolean debug, String tempPath) {
        ...
        // 增强对比度
        Core.convertScaleAbs(grad_x, abs_grad_x, alpha, beta);
