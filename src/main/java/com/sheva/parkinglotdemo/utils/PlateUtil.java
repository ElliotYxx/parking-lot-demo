...
import com.sheva.parkinglotdemo.enumtype.PlateHSV;
import com.sheva.parkinglotdemo.train.SVMTrain;
...
public class PlateUtil {
    ...
    public static String charsSegment(Mat inMat, PlateColor color, Boolean debug, String tempPath) {
        ...
        // 识别字符，计算置信度
        List<PlateRecordResult> result = Lists.newArrayList();

        // 一般来说，中文字符预测比较不准确，所以参考业界的做法，设置默认所在的省份字符，如果置信度较低， 则默认该字符
        PlateRecordResult chinese = new PlateRecordResult();
        chinese.setSort(0);
        ...
        for (int i = posi; i < sorted.size() && charCount > 0; i++, charCount--) { // 预测中文之外的字符
            ...
            PlateRecordResult chars = new PlateRecordResult();
            chars.setSort(i+1);
        ...
        for (PlateRecordResult p : result) {
            plate += p.getChars();
